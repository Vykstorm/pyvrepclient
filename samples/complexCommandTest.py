...
if clientID!=-1:
    ...
    emptyBuff = bytearray()
    res,retInts,retFloats,retStrings,retBuffer=vrep_binds.simxCallScriptFunction(clientID, 'remoteApiCommandServer', vrep_binds.sim_scripttype_childscript, 'displayText_function', [], [], ['Hello world!'], emptyBuff, vrep_binds.simx_opmode_blocking)
    ...
    res,retInts,retFloats,retStrings,retBuffer=vrep_binds.simxCallScriptFunction(clientID, 'remoteApiCommandServer', vrep_binds.sim_scripttype_childscript, 'createDummy_function', [], [0.1, 0.2, 0.3], ['MyDummyName'], emptyBuff, vrep_binds.simx_opmode_blocking)
    if res==vrep_binds.simx_return_ok:
    ...
    "return 'done'"
    res,retInts,retFloats,retStrings,retBuffer=vrep_binds.simxCallScriptFunction(clientID, "remoteApiCommandServer", vrep_binds.sim_scripttype_childscript, 'executeCode_function', [], [], [code], emptyBuff, vrep_binds.simx_opmode_blocking)
    if res==vrep_binds.simx_return_ok:
